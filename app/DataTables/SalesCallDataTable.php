...
class SalesCallDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $checkStatusAction = Role::where('name_slug','sales_calls_report')->whereRaw("find_in_set('".$status_action->id."',action_id)")->first();
        $roles = Role::where('name_slug','sales_calls_report')->first();
                ...
                foreach ($action_ids as $key => $action_id) {
                    ...
                    if (!empty($checkPermission) || Auth::user()->user_type == 'admin') {
                        if ($action->action_slug == 'edit' || $action->action_slug == 'delete' || $action->action_slug == 'view' || $action->action_slug == 'password') {
                            $btn .= '<a href="'.route("sales_calls_report.$action->action_slug",$row->id).'" class="btn btn-'.$action->class.' btn-sm" data-placement="top" data-original-title="'.$action->action_title.'" onclick="return confirm("Are you sure?")"><i class="'.$action->icon.'"></i>'.$action->action_title.'</a>&nbsp;';
                        }
                    }
                }
                return $btn;
    ...
    {
        $d = $this->data;
        if (!empty($d['from']) || !empty($d['to']) || !empty($d['customer']) || !empty($d['status']) || !empty($d['user_id'])) {
            $queryData = Lead::join('users','leads.user_id','=','users.id');

            if (!empty($d['from'])) {
                $queryData = $queryData->where('leads.date','>=',$d['from']);
            }
            if (!empty($d['to'])) {
                $queryData = $queryData->where('leads.date','<=',$d['to']);
            }
            if (!empty($d['customer'])) {
                $queryData = $queryData->where('leads.customer_id',$d['customer']);
            }
            if (!empty($d['status'])) {
                $queryData = $queryData->where('leads.status',$d['status']);
            }
            if (!empty($d['user_id'])) {
                $queryData = $queryData->where('leads.user_id',$d['user_id']);
            }
            $queryData = $queryData->select('leads.*','leads.name as customer_name','users.name as rep');
        }else{
            $queryData = Lead::join('users','leads.user_id','=','users.id')
                            ->select('leads.*','leads.name as customer_name','users.name as rep');
        }
